...
class DoubanUserUpdateSpider(CrawlSpider):
    ...
    items_id = set()
    start_urls = [
        'https://www.douban.com/people/windbo/contacts'
    ]
    ...
    def parse_contacts(self, response):
        ...
        contacts_urls = selector.xpath('//*[@id="content"]/div/div[1]/dl/dd/a/@href').extract()
        contacts = [re.match(r'^https\:\/\/www\.douban\.com\/people\/(?P<user_id>\w+)\/$', contact_url).group('user_id') for
                    contact_url in contacts_urls]

        limit = 0
        for contact in contacts:
            if limit < self.CONTACTS_LIMIT:
    ...
    def parse_user_movies(self, response):
        ...
        watched_total = re.match(r'\((?P<watched_total>\d+)\)$', total_string)
        if watched_total:
        ...
        number_of_top_movies_to_crawl = int(watched_total * self.TOP_PERCENTAGE)
